...
function buildInvoiceSnapshot(invoice) {
  ...
  const grandTotal = round2(Number(invoice?.grand_total) || taxableTotal + totalTax);
  const paidAmount = round2(Number(invoice?.paid_amount) || 0);
  const previousBalance = round2(Number(invoice?.previous_balance) || 0);
  const balance = round2(Number(invoice?.balance) || previousBalance + paidAmount - grandTotal);
...
export default function InvoicePrint() {
  ...
  const totals = buildInvoiceSnapshot(invoice);
  const isExport = Boolean(invoice.is_export) || invoice.supply_type === "EXPORT";
  ...
  return (
    <div className="invoice-print-wrapper">
      ...
      <div className={`invoice-paper ${selectedSize.cssClass}`} style={paperStyle} ref={printRef} id="invoice-print-area">
            ...
            <table className="inv-meta-table">
              <tbody>
                ...
                <tr>
                  ...
                  <td>{invoice.place_of_supply_state_name || invoice.place_of_supply_country || "-"}</td>
                </tr>
                <tr>
                  <td>Supply Type</td>
                  <td>{(invoice.supply_type || "INTRA_STATE").replace(/_/g, " ")}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="inv-panel-grid">
          <div className="inv-panel">
            <div className="inv-panel-title">Bill To</div>
            <div className="inv-party-name">{invoice.customer_salutation} {invoice.customer_name}</div>
            {invoice.customer_billing_address && <div className="inv-panel-text">{invoice.customer_billing_address}</div>}
            {invoice.customer_mobile && <div className="inv-panel-text">Phone: {invoice.customer_mobile}</div>}
            {invoice.customer_gstin && <div className="inv-panel-text">GSTIN: {invoice.customer_gstin}</div>}
          </div>

          <div className="inv-panel">
            <div className="inv-panel-title">Ship To</div>
            <div className="inv-panel-text">{invoice.customer_shipping_address || invoice.customer_billing_address || "-"}</div>
                ...
                <tr key={item.id || `${item.product_id}_${index}`}>
                  ...
                  <td className="text-end">
                    {Number(item.cgst_rate) > 0 ? (
                      <>
                        <div className="inv-tax-amt">₹ {formatCurrency(item.cgst_amount)}</div>
                  ...
                  <td className="text-end">
                    {Number(item.sgst_rate) > 0 ? (
                      <>
                        <div className="inv-tax-amt">₹ {formatCurrency(item.sgst_amount)}</div>
                  ...
                  <td className="text-end">
                    {Number(item.igst_rate) > 0 ? (
                      <>
                        <div className="inv-tax-amt">₹ {formatCurrency(item.igst_amount)}</div>
          ...
          {invoice.items.map((item, index) => (
            <div key={item.id || `${item.product_id}_${index}`} className="inv-receipt-item">
              <div className="inv-receipt-row">
                ...
                <span className="inv-receipt-total">₹ {formatCurrency(item.total_value)}</span>
              </div>
              <div className="inv-receipt-row inv-receipt-meta">
                <span>Qty: {item.quantity} × ₹ {formatCurrency(
                  invoice.price_includes_gst && item.total_value && item.quantity
                    ? round2(item.total_value / item.quantity)
                    : item.rate
                )}</span>
                <span>Tax: ₹ {formatCurrency((item.cgst_amount || 0) + (item.sgst_amount || 0) + (item.igst_amount || 0))}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="inv-footer-grid">
          <div className="inv-footer-left">
            ...
            <div className="inv-note-block">
              <div className="inv-panel-title">Terms & Conditions</div>
              <div className="inv-panel-text" style={{ whiteSpace: "pre-line" }}>{company?.terms_and_conditions || invoice.notes || "Goods once sold will not be taken back without prior approval."}</div>
            </div>

            {isExport && (
              <div className="inv-note-block">
                <div className="inv-panel-title">Export Declaration</div>
                <div className="inv-panel-text">Supply Meant for Export under LUT / Bond - Zero Rated Supply</div>
              </div>
            )}
          ...
          <div className="inv-summary">
            ...
            <div className="inv-summary-row">
              ...
              <span>₹ {formatCurrency(totals.taxableTotal)}</span>
            </div>
            <div className="inv-summary-row">
            ...
            {totals.discount > 0 && (
              <div className="inv-summary-row">
                <span>Discount {invoice.discount_type === "PERCENTAGE" ? `(${invoice.discount_input}%)` : ""}</span>
                <span>- ₹ {formatCurrency(totals.discount)}</span>
              </div>
            )}
            <div className="inv-summary-row inv-summary-grand">
              <span>Grand Total</span>
              <span>₹ {formatCurrency(totals.grandTotal)}</span>
            </div>
            <div className="inv-summary-row">
              <span>Previous Balance</span>
              <span>₹ {formatCurrency(Math.abs(totals.previousBalance))}</span>
            </div>
            <div className="inv-summary-row">
      ...
      <style>{`
        ...
        .page-a3 { page: invoice-a3; }
        .page-a4 { page: invoice-a4; }
        .page-a5 { page: invoice-a5; }
        .page-thermal { page: invoice-thermal; }
