...
function mapVendorToForm(row) {
  return {
    salutation: row.salutation || "M/s.",
    name: row.name || "",
    mobile: row.mobile || "",
    email: row.email || "",
    gstin: row.gstin || "",
  ...
  const columns = [
    ...
    { key: "mobile", label: "Mobile", style: { width: "120px" } },
    {
      ...
      render: (row) => {
        ...
        if (balance === 0) return <span className="text-muted">Rs.0.00</span>;
        if (balance < 0) return <span className="text-danger fw-medium">Due: Rs.{Math.abs(balance).toFixed(2)}</span>;
        return <span className="text-success fw-medium">Adv: Rs.{balance.toFixed(2)}</span>;
      },
    },
    {
      key: "gstin",
      ...
      render: (row) => (
        row.gstin ? (
          ...
          <span className="text-muted">{isIndianCountry(row.country || "India") ? "Domestic" : "Import"}</span>
        )
      ),
    },
    {
              ...
              <div className="modal-content border-0 shadow">
                <div className="modal-header">
                  <h6 className="modal-title fw-semibold">
                    <i className="fa-solid fa-truck me-2 text-primary" />
                    {editId ? "Edit" : "New"} Vendor
                  </h6>
